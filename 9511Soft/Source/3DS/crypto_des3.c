...
#include "crypto_des3.h"
#include "crypto_intern.h"
...
static const unsigned char pc1[256] = {
...
static const unsigned char rs[256] = {
...
static const unsigned long pc2[1024] = {
...
/* -----------------------------------------------------  S-box lookup tables.  */
static const unsigned long S1[64] = {
...
static const unsigned long S2[64] = {
...
static const unsigned long S3[64] = {
...
static const unsigned long S4[64] = {
...
static const unsigned long S5[64] = {
...
static const unsigned long S6[64] = {
...
static const unsigned long S7[64] = {
...
static const unsigned long S8[64] = {
 ...
 */
#define T1(x)   ptr[2 * (x) + 0]
#define T2(x)   ptr[2 * (x) + 1]
#define T3(x)   ptr[2 * (x) + 2]
#define T4(x)   ptr[2 * (x) + 3]

#define CRYPTO_DES_PC2(a, b, c, d) (T4(d) | T3(c) | T2(b) | T1(a))
...
crypto_des_expand_key(unsigned long* p_exp_key, const unsigned char *p_key) {
  unsigned long  a;
  unsigned long  b;
  unsigned long  c;
  unsigned long  d;
  unsigned long  w;
  const unsigned long* ptr = pc2;


  d = p_key[4]; d &= 0x0e; d <<= 4; d |= p_key[0] & 0x1e; d = pc1[d];
  c = p_key[5]; c &= 0x0e; c <<= 4; c |= p_key[1] & 0x1e; c = pc1[c];
  b = p_key[6]; b &= 0x0e; b <<= 4; b |= p_key[2] & 0x1e; b = pc1[b];
  a = p_key[7]; a &= 0x0e; a <<= 4; a |= p_key[3] & 0x1e; a = pc1[a];

  p_exp_key[15 * 2 + 0] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d];
  p_exp_key[14 * 2 + 0] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[13 * 2 + 0] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[12 * 2 + 0] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[11 * 2 + 0] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[10 * 2 + 0] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 9 * 2 + 0] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 8 * 2 + 0] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c];
  p_exp_key[ 7 * 2 + 0] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 6 * 2 + 0] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[ 5 * 2 + 0] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 4 * 2 + 0] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[ 3 * 2 + 0] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 2 * 2 + 0] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[ 1 * 2 + 0] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b];
  p_exp_key[ 0 * 2 + 0] = CRYPTO_DES_PC2(b, c, d, a);
  ...
  d = p_key[0]; d &= 0xe0; d >>= 4; d |= p_key[4] & 0xf0; d = pc1[d + 1];
  c = p_key[1]; c &= 0xe0; c >>= 4; c |= p_key[5] & 0xf0; c = pc1[c + 1];
  b = p_key[2]; b &= 0xe0; b >>= 4; b |= p_key[6] & 0xf0; b = pc1[b + 1];
  a = p_key[3]; a &= 0xe0; a >>= 4; a |= p_key[7] & 0xf0; a = pc1[a + 1];

  /* Check if second half is weak.                                            */
  w |= (a ^ c) | (b ^ d) | (rs[a] ^ c) | (b ^ rs[d]);

  p_exp_key[15 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d];
  p_exp_key[14 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[13 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[12 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[11 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[10 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 9 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 8 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c];
  p_exp_key[ 7 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 6 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[ 5 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 4 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[ 3 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 2 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[ 1 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b];
  p_exp_key[ 0 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a);
...
/* --------------------------------------------- 扩展 Triple DES 加密算法密钥。 */
static void
crypto_des3_expand_key(unsigned long* p_exp_key, const unsigned char *p_key) {
  unsigned long  a;
  unsigned long  b;
  unsigned long  c;
  unsigned long  d;
  ...
  p_exp_key[ 0 * 2] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d];
  p_exp_key[ 1 * 2] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 2 * 2] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 3 * 2] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 4 * 2] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 5 * 2] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 6 * 2] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 7 * 2] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c];
  p_exp_key[ 8 * 2] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 9 * 2] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[10 * 2] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[11 * 2] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[12 * 2] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[13 * 2] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[14 * 2] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b];
  p_exp_key[15 * 2] = CRYPTO_DES_PC2(b, c, d, a);
  ...
  p_exp_key[ 0 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d];
  p_exp_key[ 1 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 2 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 3 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 4 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 5 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c]; b = rs[b];
  p_exp_key[ 6 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a); a = rs[a]; d = rs[d];
  p_exp_key[ 7 * 2 + 1] = CRYPTO_DES_PC2(d, a, b, c); c = rs[c];
  p_exp_key[ 8 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[ 9 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[10 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[11 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[12 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b]; a = rs[a];
  p_exp_key[13 * 2 + 1] = CRYPTO_DES_PC2(a, b, c, d); d = rs[d]; c = rs[c];
  p_exp_key[14 * 2 + 1] = CRYPTO_DES_PC2(c, d, a, b); b = rs[b];
  p_exp_key[15 * 2 + 1] = CRYPTO_DES_PC2(b, c, d, a);
...
crypto_des3_set_key(crypto_des3_ctx_t*   p_ctx,
  ...
  unsigned long* p_exp_key = p_ctx->expkey;


  crypto_des_expand_key(p_exp_key, p_key);
  p_exp_key += CRYPTO_DES_EXPKEY_WORDS;
  p_key     += CRYPTO_DES_KEY_SIZE;
  crypto_des3_expand_key(p_exp_key, p_key);
  p_exp_key += CRYPTO_DES_EXPKEY_WORDS;
  p_key     += CRYPTO_DES_KEY_SIZE;
  crypto_des_expand_key(p_exp_key, p_key);
...
crypto_des3_encrypt(const crypto_des3_ctx_t* p_ctx,
  ...
  const unsigned long* p_exp_key = p_ctx->expkey;
